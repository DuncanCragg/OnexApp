...
public:
  ...
  void drawGUI();
  void drawView();
  void drawObjectProperties(char* path, bool locallyEditable, uint16_t width);
  void drawNestedObjectProperties(char* path, bool locallyEditable, uint16_t width, uint16_t height);
  void drawNewPropertyValueEditor(char* path, char* key, char* val, bool locallyEditable, uint16_t width, uint16_t height);
  void drawNewValueOrObjectButtons(char* path, uint16_t width);
  object* createNewObjectLikeOthers(char* path);
  void drawNewPropertyCombo(char* path);
  void drawObjectHeader(char* path, bool locallyEditable, uint16_t width);
  void drawNestedObjectPropertiesList(char* path, bool locallyEditable, uint16_t width, uint16_t height);
  void drawPropertyValue(char* path, char* key, char* val, bool locallyEditable, uint16_t width);
  void drawPropertyList(char* pathkey, char* key, bool locallyEditable, uint16_t width);
